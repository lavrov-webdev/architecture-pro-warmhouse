...
Для всех сервисов будет использоваться синхронный api
Для запуска команд будет использовать асинхронное api, так как команды могут вставать в очередь в случае большой нагрузки

### 2. Документация API

Openapi спецификации находятся в папке schemas
...
3) Кроме того для smart_home приложения требуется база данных — добавьте в docker-compose сервис миграций, который после успешного старта postgres применит схему из `apps/smart_home/init.sql` к базе `smarthome`.
