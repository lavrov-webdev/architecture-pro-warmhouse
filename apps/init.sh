...
for i in {1..30}; do
  if podman exec smarthome-postgres pg_isready -U postgres > /dev/null 2>&1; then
...
if podman exec smarthome-postgres psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='smarthome'" | grep -q 1; then
  ...
  if [ -f "$SQL_FILE" ]; then
    ...
    tail -n +7 "$SQL_FILE" | podman exec -i smarthome-postgres psql -U postgres -d smarthome -v ON_ERROR_STOP=1
    echo "Schema ensured."
  else
  ...
  if [ -f "$SQL_FILE" ]; then
    cat "$SQL_FILE" | podman exec -i smarthome-postgres psql -U postgres -v ON_ERROR_STOP=1
...
echo "To stop the services, run: podman-compose down"